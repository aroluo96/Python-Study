...
def answer_six():
    Top15 = answer_one() 
    percentage = Top15['% Renewable'].max(axis=0)
    country = Top15.loc[Top15['% Renewable'] == percentage]
    return (country.index.tolist()[0], percentage)
...
def answer_seven():
    ...
    max_citation_ratio = Top15['Ratio'].max(axis=0)
    country_citation = Top15.loc[Top15['Ratio'] == max_citation_ratio]
    return (country_citation.index.tolist()[0], max_citation_ratio)
...
def answer_eight():
    ...
    answer = Top15.sort_values(by = 'Estimate Population', ascending = False).index.tolist()[2]
    return answer
answer_eight()
...
def answer_ten():
    Top15 = answer_one()
    Top15['Dummy'] = [1 if x >= Top15['% Renewable'].median() else 0 for x in Top15['% Renewable']]
    return Top15['Dummy']
answer_ten()
...
def answer_eleven():
    ...
    copy_top15 = Top15.groupby(['Continent'])['Estimate Population'].agg({'size': np.size, 'sum': np.sum, 'mean': np.mean,'std': np.std})
    return copy_top15
...
def answer_thirteen():
    Top15 = answer_one()
    Top15['Estimate Population'] = Top15['Energy Supply']/Top15['Energy Supply per Capita']
    return Top15['Estimate Population'].map('{:,}'.format)
answer_thirteen()
